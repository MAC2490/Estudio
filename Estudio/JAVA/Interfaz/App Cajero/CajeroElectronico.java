...
public class CajeroElectronico{
	...
	public void consultarSaldoTarjeta(TarjetaDebito saldoTarjeta, String contrasena){
		if (saldoTarjeta.verificarPassword(contrasena)){
			System.out.println("SALDO: "+saldoTarjeta.getSaldo());
			this.registrarLog("CONSULTA_SALDO", saldoTarjeta.getNumero(), 0, "ok:La consulta se realizo con exito");
		}else{
			this.registrarLog("CLAVE_INCORRECTA", saldoTarjeta.getNumero(), 0, "ERROR: congtraseña incorrecta");
			System.out.println("Error contraseña incorrecta");
		}
	}

	public void cambiarClave(TarjetaDebito tarjeta, String clave, String nuevaClave){
		if (tarjeta.verificarPassword(clave)){
			tarjeta.setPassword(nuevaClave);
			System.out.println("Su clave fue cambiada con exito");
			this.registrarLog("Cambio_de_clave ", tarjeta.getNumero(), 0, "ok: Su contraseña fue cambiada con exito");
		}else{
			this.registrarLog("CLAVE_INCORRECTA", tarjeta.getNumero(), 0, "ERROR: congtraseña incorrecta");
	...
	public void transferirDineroTarjeta(TarjetaDebito tarjeta, int cant_10, int cant_20, int cant_50, int cant_100){
		...
		this.cant_50 += cant_50;
		this.cant_100 += cant_100;
		int nuevoValor = (cant_10*10000)+(cant_20*20000)+(cant_50*50000)+(cant_100*100000);
		tarjeta.setSaldo(nuevoValor);
		this.cant_dinero_disponible += nuevoValor;
		this.registrarLog("CONSIGNACION_SALDO", tarjeta.getNumero(),0,"ok: Se consigno correctamente");
	}
